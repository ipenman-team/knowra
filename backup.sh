...
BACKUP_FILE="$BACKUP_DIR/knowra_$TIMESTAMP.sql"
...
if $COMPOSE -f docker-compose.prod.yml exec -T postgres pg_dump -U knowra knowra > "$BACKUP_FILE"; then
  ...
  find "$BACKUP_DIR" -name "knowra_*.sql.gz" -mtime +$KEEP_DAYS -delete

  echo "[$(date '+%Y-%m-%d %H:%M:%S')] 最近的备份:" | tee -a "$LOG_FILE"
  ls -lh "$BACKUP_DIR"/knowra_*.sql.gz 2>/dev/null | tail -5 | tee -a "$LOG_FILE"
