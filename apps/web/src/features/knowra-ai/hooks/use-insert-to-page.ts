...
import { markdownToSlateValue } from '@knowra/slate-converters';
  ...
  const suggestedTitle = useMemo(
    () =>
      buildPageTitle({
        ...
        fallbackTitle: t('knowraAiInsert.untitled'),
      }),
  ...
  const insertToPage = useCallback(
    async (spaceId: string, pageId: string): Promise<boolean> => {
      ...
      try {
        ...
        toast.success(t('knowraAiInsert.insertSuccess'));
        return true;
      } catch {
        toast.error(t('knowraAiInsert.insertFailed'));
        return false;
      } finally {
        setPending(false);
      }
      ...
      try {
        ...
        if (publish) {
          try {
            ...
            toast.error(t('knowraAiInsert.createPublishFailed'));
            return false;
          }
        }
        ...
        toast.success(t('knowraAiInsert.createSuccess'));
        return true;
      } catch {
        toast.error(t('knowraAiInsert.createFailed'));
        return false;
      } finally {
        setPending(false);
      }
