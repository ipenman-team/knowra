...
import { useInsertToPage } from '@/features/knowra-ai/hooks/use-insert-to-page';
import { KnowraAiCreateDocumentModal } from './knowra-ai-create-document-modal';
...
export function KnowraAiInsertToPageButton(props: {
  spaces: SpaceOption[];
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button type="button" size="sm" variant="outline" disabled={pending}>
            ...
            {t('knowraAiInsert.insertToDoc')}
            <ChevronDown className="h-4 w-4 opacity-70" />
          </Button>
        </DropdownMenuTrigger>

        <DropdownMenuContent align="end" className="w-44">
          <DropdownMenuLabel>{t('knowraAiInsert.selectSpace')}</DropdownMenuLabel>
          <DropdownMenuSeparator />
          <DropdownMenuGroup>
            <DropdownMenuItem
              disabled={pending}
            ...
            >
              ...
              {t('knowraAiInsert.newSpace')}
            </DropdownMenuItem>
          </DropdownMenuGroup>
          <DropdownMenuSeparator />

          {props.spaces.length === 0 ? (
            <DropdownMenuItem disabled>{t('knowraAiInsert.noSpaces')}</DropdownMenuItem>
          ) : (
                  ...
                  <DropdownMenuSubContent className="w-52">
                    ...
                    {!loading && loadFailed ? (
                      <>
                        ...
                        >
                          {t('knowraAiInsert.loadPagesFailed')}
                        </DropdownMenuItem>
                      </>
                    ) : null}
                        ...
                        <DropdownMenuGroup>
                          <div className="max-h-56 overflow-y-auto">
                            {pages.map((page) => (
                              ...
                              >
                                {page.title?.trim() || t('knowraAiInsert.untitled')}
                              </DropdownMenuItem>
                            ))}
                          </div>
                        </DropdownMenuGroup>
