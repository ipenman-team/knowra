...
import { KnowraAiHeader } from '@/features/knowra-ai/components/knowra-ai-header';
import { KnowraAiEmptyState } from '@/features/knowra-ai/components/knowra-ai-empty-state';
import { KnowraAiMessageList } from '@/features/knowra-ai/components/knowra-ai-message-list';
import { KnowraAiComposer } from '@/features/knowra-ai/components/knowra-ai-composer';
import { hasAbortName } from '@/features/knowra-ai/components/utils';
...
export function KnowraAiContent(props: {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const cfg = await knowraAiApi.getConversationSources(
  ...
  async function persistSourcesNow(): Promise<void> {
    ...
    const saved = await knowraAiApi.updateConversationSources(
      props.conversationId,
      payload,
    );
  ...
  async function sendQuestion(
    ...
    const baseMessages: KnowraAiMessage[] = [
      ...props.messages,
    ...
    if (attachments.length > 0) {
      try {
        const upload = await knowraAiApi.uploadAttachments({
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col">
      <KnowraAiHeader title={props.title} copied={copied} onShare={handleShare} />
