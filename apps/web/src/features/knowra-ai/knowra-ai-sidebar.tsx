...
export const KnowraAiSidebar = memo(function KnowraAiSidebar(props: {
  conversations: KnowraAiConversation[];
  activeId: string;
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<KnowraAiConversation | null>(null);
  ...
  function handleOpenDelete(target: KnowraAiConversation) {
  ...
  return (
    <Sidebar collapsible="icon" variant="sidebar" className="h-full">
      <SidebarHeader className="p-4">
        <InputGroup>
          <InputGroupInput
            placeholder={t('knowraAiSidebar.searchPlaceholder')}
            value={keyword}
      ...
      <SidebarContent className="gap-0">
        <SidebarGroup className="pt-0">
          <SidebarGroupLabel>{t('knowraAiSidebar.history')}</SidebarGroupLabel>
          <SidebarMenu>
            {items.length === 0 ? (
              <div className="px-2 py-2 text-sm text-muted-foreground">
                {t('knowraAiSidebar.empty')}
              </div>
            ) : (
              items.map((c) => (
                <SidebarMenuItem key={c.id}>
                  ...
                  >
                    ...
                    {renamingId === c.id ? (
                      ...
                      <span>
                        {c.title || t('knowraAiSidebar.untitledConversation')}
                      </span>
                    )}
                  </SidebarMenuButton>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      >
                        {c.pinned
                          ? t('knowraAiSidebar.unpin')
                          : t('knowraAiSidebar.pin')}
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
      ...
      <SidebarFooter>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            ...
            >
              <Plus />
              <span>{t('knowraAiSidebar.newConversation')}</span>
            </SidebarMenuButton>
          </SidebarMenuItem>
        </SidebarMenu>
      </SidebarFooter>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('knowraAiSidebar.deleteTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {deleteTarget
                ? `${t('knowraAiSidebar.deletePrompt')}“${
                    deleteTarget.title || t('knowraAiSidebar.untitledConversation')
                  }”${t('knowraAiSidebar.deletePromptSuffix')}`
                : ''}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
